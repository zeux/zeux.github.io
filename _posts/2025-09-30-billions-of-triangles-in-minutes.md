...
To build the hierarchical structure in question, we need clusterization (to split meshes into clusters), partitioning (to group clusters together) and simplification (to reduce a cluster group to fewer triangles). Fortunately, [meshoptimizer](https://github.com/zeux/meshoptimizer/) provides algorithms for all three.
