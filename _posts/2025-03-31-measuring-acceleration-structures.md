...
As is clear from the name, BVH8 node stores 8 children; instead of using fp16 for box bounds, it stores the box corners in a special format[^15] with 12-bit mantissa and shared 8-bit exponent between all corners, plus a full fp32 origin corner. This adds up to 128 bytes - from the memory perspective it's just as much as two fp16 BVH4 nodes from RDNA2/3, but it should permit the full fp32 range of bounding box values - fp16 box nodes, for example, could not represent geometry with coordinates outside of +-64K! - so I would expect that RDNA4 BVH data does not use any BVH4 nodes short of some other limitations, and this allows AMD to embed other sorts of data into the box node, such as the OBB index for their new rotation support, and the parent pointer (which previously, as you recall, had to be allocated separately).

Primitive node is somewhat similar to triangle node, but it's larger (128 bytes) and much more versatile: it can store a variable number of triangle pairs per node, and does that using what seems like a micro-meshlet format, where triangle pairs use vertex indices, with a separate section of the 128-byte packet storing the vertex positions - using a variable amount of bits per vertex for position storage. For the vertex positions, it looks like all bits inside coordinates for a single primitive are split into three parts: prefix (must be the same across all floats for the same axis), value (variable width), trailing zeroes. For fp16 source positions, I would expect prefix storage to remove the initial segment of bits shared between the positions which would be close together in memory, and most of the trailing fp32 bits to be zero. It would probably be reasonable to expect around 30-33 bits per vertex (3 * 10-bit mantissas, with most of the exponent bits shared and the trailing zeroes removed) with that setup.

The triangle pair indices are encoded using 4-bit per index, with a few other bits used for other fields; primitive indices are stored as a delta from a single base value inside the primitive node, similarly to positions. Notably, the triangle pair has three independent indices for three corners - so it looks like the pair does not necessarily have to share the geometric edge, which presumably improves the efficiency at which geometry can be converted to this format at a small cost of 8 extra bits for every second triangle.

All of this makes it very difficult to correctly estimate the optimal storage capacity of such a node. If ~30 bits per vertex for position storage is accurate, then 16 vertices will take 60 bytes of storage; a 4x4 vertex grid requires 16 vertices and 18 triangles. With each triangle occupying 29 bits, 18 triangles would take 65 bytes. With at least a few more bytes required for additional anchors and such, this probably can't quite fit - and in fact, it looks like triangle pair count is limited to 8. So realistically we are probably looking at 16 triangles representing a 3x5 vertex grid (15 vertices) as a best case[^16], for, at best, ~8.5 bytes/triangle.
...
- 128-byte primitive nodes with 15 triangles/node = 8.5 bytes/triangle
- 128-byte box nodes, 1/7th of 1/15th of triangles = 1.2 bytes/triangle

... for a grand total of 9.7 bytes/triangle. Now, with the *actual* numbers of ~48 bytes/triangle, this is clearly a wildly unrealistic goal:
...
- It is likely that the hardware acceleration units can sustain a lower intersection rate than a full primitive node/clock, likely taking many cycles to process the densest node possible - which may degrade trace throughput.

Between these two factors, it's hard to estimate the realistic expected memory consumption - but it seems plausible that we will see significant reduction of BVH sizes with future driver updates (perhaps from 48 bytes/triangle to something closer to 32?). Additionally, note that Bistro geometry has a lot of triangles with odd shapes and in general is not particularly uniform or dense. It's possible that on denser meshes the effective bytes/triangle ratio is going to be closer to the theoretical optimum - exploring denser meshes is left as an exercise to the reader!
...
[^6]: These numbers were captured using official Intel drivers on Windows, *not* Mesa on Linux. I don't have Intel's Linux numbers handy, and don't feel like replugging the GPU again.
...
[^12]: Interested readers are encouraged to explore this topic further; on AMD hardware, you can use [RRA](https://gpuopen.com/radeon-raytracing-analyzer/) to analyze the BVH as well as traversal efficiency characteristics for your workload.
...
[^16]: This math is similar to meshlet configurations described [in an earlier post](https://zeux.io/2023/01/16/meshlet-size-tradeoffs/).
