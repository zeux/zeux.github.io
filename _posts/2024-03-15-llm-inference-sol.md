...
> This post is an experiment in an attempt to publish more posts that are shorter, take less time to write, and may not always be as thoroughly investigated and structured as previously published articles on this blog.[^1] There is not going to be a clear dividing line - in fact, this post is much longer than the shorter posts I'd normally write, but you've been warned that shorter posts are incoming :)
...
> This blog post is trying hard to be short but failing! Ah well.
...
[^1]: I wrote more about the motivation for this in https://cohost.org/zeux/post/309475-write-less-to-write -- but at the time I was assuming Cohost would be the platform for this. Since Cohost has an uncertain future and difficulties with discovery, I plan to instead post content like this here, as well as repost some older Cohost posts that are technical in nature.
